...
def recompile(alpha,tau,c):
    ...
    filein = open( f'/mnt/home/sakkosjo/nufeb-cyano-e-coli/templates/fix_bio_kinetics_monodSUC.txt' )
    #read it
...
def func(x):
    ...
    ax.plot(df.IPTG,df.Sucrose)
    ax.set_xlabel('IPTG (mM)')
    ax.set_ylabel('Sucrose (mM)')
    f.tight_layout()
    f.savefig(f'/mnt/home/sakkosjo/nufeb-cyano-e-coli/simulation-data/se-optSUC-{Nfeval}.png')
...
checkpoint_saver = CheckpointSaver('/mnt/home/sakkosjo/nufeb-cyano-e-coli/checkpoints/checkpoint-se-suc-icer.pkl', compress=9)
n_calls = 200
