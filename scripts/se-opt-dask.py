...
text = f'nufeb-seed --n 10 --cells 10,0 --t 8700 --iptg 1 --mucya 1.89e-5'
os.system(text)
...
def run_simulation(path):
    ...
    p=Path('.')
    lmp = lammps()
    lmp.file(str(list(p.glob('**/*.lammps'))[0]))
...
with Parallel(n_jobs=-1) as parallel:
    parallel(delayed(run_simulation)(folder) for folder in folders)
...
futures=client.map(run_simulation,folders,pure=False)
progress(futures)
def od_func(x):
